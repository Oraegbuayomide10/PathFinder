...
from PATHFinder.utils.loss_optim_LR_utils import calculate_eval_metrics
import torch
import numpy as np
import os
import glob
...
def evaluate_model(model, dataloader, device="cpu"):
    ...
    with torch.no_grad():
        for images, annotations, _, _, _ in dataloader:
            images = images.float()
            ...
            preds = torch.argmax(
                outputs, dim=1
            )  # Take the argmax to get predicted classes
...
def main(args):
    ...
    eval_images = sorted(
        glob.glob(os.path.join(args.images_dir, f"*{args.images_format}"))
    )
    eval_labels = sorted(
        glob.glob(os.path.join(args.labels_dir, f"*{args.labels_format}"))
    )

    # Dataset and Dataloader
    eval_dataset = SegmentationDataset(
        images_list=eval_images, labels_list=eval_labels, num_classes=2, train=False
    )
    ...
    os.makedirs(preds_dir, exist_ok=True)  # create save dir
    visualize_batch_predictions(predictions, labels, preds_dir)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate PATHFinder model.")
    parser.add_argument(
        "--model_checkpoint", required=True, help="Path to the model checkpoint file."
    )
    parser.add_argument(
        "--images_dir", required=True, help="Directory containing images to evaluate."
    )
    parser.add_argument(
        "--labels_dir",
        required=True,
